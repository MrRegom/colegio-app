...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Función para el botón de firma de jefatura
    if (signatureButton) {
        signatureButton.addEventListener('click', function(e) {
            // No hacer nada aquí, ya que la funcionalidad real se maneja en el template
            // Solo prevenir el comportamiento predeterminado para evitar que se envíe el formulario
            e.preventDefault();
            
        });
    }
    
    // Función para cargar el formulario en modo lectura
    function loadReadOnlyForm() {
        const urlParams = new URLSearchParams(window.location.search);
        
        // Verificar si estamos en modo solo lectura
        if (urlParams.get('readonly') === 'true' && profileForm) {
            console.log("Cargando formulario en modo solo lectura");
            
            // Cargar los valores de la URL a los campos del formulario
            for (const [key, value] of urlParams.entries()) {
                if (key !== 'readonly') {
                    const element = document.getElementById(key);
                    if (element) {
                        // Establecer el valor según el tipo de elemento
                        ...
                        // Para checkboxes de sistema[], marcar los correspondientes
                        const checkboxes = document.querySelectorAll('input[name="sistema[]"]');
            ...
            // Establecer formulario en modo de solo lectura
            ...
            // Ocultar botón de enviar y mostrar solo el de firma
            const submitBtn = document.getElementById('submitToLeadership');
            const signatureBtn = document.getElementById('leadershipSignature');
            
            if (submitBtn) submitBtn.style.display = 'none';
            if (signatureBtn) {
                signatureBtn.classList.add('btn-primary');
                signatureBtn.classList.remove('btn-secondary');
            }
            
            // Cambiar el título del banner
            const bannerTitle = document.querySelector('.card.bg-primary h2');
            if (bannerTitle) {
                bannerTitle.textContent = 'Solicitud de Firma - Formulario de Perfiles';
            }
        }
    }
});
